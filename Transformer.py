...
# 建立資料夾存圖
os.makedirs("hysteresisloops_Normalize_Transformer", exist_ok=True)
...
def plot_hysteresis(real_dis, real_force, gen_dis, gen_force, epoch):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"hysteresisloops_Normalize_Transformer/hysteresis_comparison_epoch_{epoch}.png")
    plt.close()
